...
from sqlalchemy_utils.functions.database import _set_url_database, _sqlite_file_exists

from .database import Base, get_engine
from .orm import MAIN_SCHEMA, HISTORY_SCHEMA
from .triggers import trf_update_server_time
    ...
    try:
        if dialect_name == "postgresql":
            ...
            for db in (database, "postgres", "template1", "template0", None):
                ...
                try:
                    result = await _get_scalar_result(engine, sa.text(text))
                    return bool(result)
...
async def create_database(url, encoding="utf8", template=None):
    exist_db = await database_exists(url)
    if exist_db:
        return
    url = make_url(url)
    database = url.database
...
async def innit_db():
    ...
    async with engine.begin() as conn:
        await conn.execute(CreateSchema(MAIN_SCHEMA, if_not_exists=True))
        await conn.execute(CreateSchema(HISTORY_SCHEMA, if_not_exists=True))

        # Создаем триггер на апдейт server_time любой записи
        event.listen(Base.metadata, "before_create", trf_update_server_time.execute_if(dialect="postgresql"))
